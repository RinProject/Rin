...
module.exports = {
	async run(message, args, colors) {
		...
		let role = message.guild.roles.resolve(
			((await get(db, 'SELECT role FROM expRole WHERE guild = (?);', [message.guild.id])) || {})
				.role
		);
		if (!role) {
			...
			await run(db, 'INSERT OR REPLACE INTO expRole(guild, role) VALUES((?), (?));', [
				message.guild.id,
				role.id,
			]);
		}
