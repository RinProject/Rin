...
module.exports = {
	async run(message, args, colors) {
		...
		if (reason && member) {
			...
			db.run(
				sql,
				[id, member.id, message.author.id, reason, message.guild.id, timestamp.toString()],
				(err) => {
					if (err) throw err;
					db.all(
						`SELECT modLogChannel FROM logs WHERE guild = "${message.guild.id}"`,
						(err, rows) => {
			...
			db.run(
				'UPDATE warnings SET active = 0 WHERE guild = (?) AND id = (?)',
			...
			db.run(
				'UPDATE warnings SET active = 1 WHERE guild = (?) AND id = (?)',
				[message.guild.id, args[2]],
				(err) => {
					if (err)
						return message.channel.send({
