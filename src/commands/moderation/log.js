		...
		if (args[1] == 'channel') {
			if (args[2] == 'mod')
				db.run(`SELECT * FROM logs WHERE guild = ${message.guild.id}`, (err, column) => {
					...
					if (column != undefined)
						return message.channel.send(
							'Please set a default logging channel with: `log channel #channeName`'
						);

					db.run(
						`REPLACE INTO logs(guild, modLogChannel) VALUES (${message.guild.id}, ${
							message.mentions.channels.first().id
						}`,
						(err) => {
							if (err) throw err;
							message.channel.send(
								`Logging channel set to: <#${message.channels.mentions.first().id}>`
							);
						}
					);
				});
				...
				db.run(`SELECT * FROM logs WHERE guild = ${message.guild.id}`, (err, column) => {
					...
					db.run(
						`REPLACE INTO logs(guild, messageLogChannel) VALUES (${message.guild.id}, ${
							message.mentions.channels.first().id
						}`,
						(err) => {
							if (err) throw err;
				...
				db.run(`SELECT * FROM logs WHERE guild = ${message.guild.id}`, (err, column) => {
					...
					db.run(
						`REPLACE INTO logs(guild, serverLogChannel) VALUES (${message.guild.id}, ${
							message.mentions.channels.first().id
						}`,
						(err) => {
							if (err) throw err;
				...
				db.run(`SELECT * FROM logs WHERE guild = ${message.guild.id}`, (err, column) => {
					...
					if (column != undefined);
					message.channel.send(
						'No logging is currently enabled, please use the command `log enable all` too enable all logging.'
					);
					if (!message.mentions.channels.first()) return;
					db.run(
						`INSERT OR REPLACE INTO logs(guild, modLogChannel, messageLogChannel, serverLogChannel) VALUES (${
							message.guild.id
						}, ${message.mentions.channels.first().id}, ${message.mentions.channels.first().id}, ${
							message.mentions.channels.first().id
						})`,
						(err) => {
							if (err) throw err;
							message.channel.send(
								`Logging channel set to <#${message.mentions.channels.first().id}>`
							);
						}
					);
				});
			...
			if (args[2] == 'all')
				...
				db.run(
					`UPDATE logs SET channelPinsUpdate = true,messageDelete = true,messageDeleteBulk = true,messageReactionAdd = true,messageReactionRemove = true,messageReactionRemoveAll = true,messageReactionRemoveEmoji = true,messageUpdate = true,inviteCreate = true,inviteDelete = true WHERE guild = "${message.guild.id}"`,
					(err) => {
						if (err) throw err;
					}
				);
				...
				db.run(
					`UPDATE logs SET channelCreate = true,channelDelete = true,channelUpdate = true,emojiCreate = true,emojiDelete = true,emojiUpdate = true,guildUpdate = true,guildIntegrationsUpdate = true,roleCreate = true,roleDelete = true,roleUpdate = true,webhookUpdate = true WHERE guild = "${message.guild.id}"`,
					(err) => {
						if (err) throw err;
					}
				);
				...
				db.run(
					`UPDATE logs SET guildBanAdd = true,guildBanRemove = true,guildMemberAdd = true,guildMemberRemove = true,guildMemberUpdate = true,userUpdate = true WHERE guild = "${message.guild.id}"`,
					(err) => {
						if (err) throw err;
					}
				);
			...
			if (args[2] == 'all')
				...
				db.run(
					`UPDATE logs SET channelPinsUpdate = false,messageDelete = false,messageDeleteBulk = false,messageReactionAdd = false,messageReactionRemove = false,messageReactionRemoveAll = false,messageReactionRemoveEmoji = false,messageUpdate = false,inviteCreate = false,inviteDelete = false WHERE guild = "${message.guild.id}"`,
					(err) => {
						if (err) throw err;
					}
				);
				...
				db.run(
					`UPDATE logs SET channelCreate = false,channelDelete = false,channelUpdate = false,emojiCreate = false,emojiDelete = false,emojiUpdate = false,guildUpdate = false,guildIntegrationsUpdate = false,roleCreate = false,roleDelete = false,roleUpdate = false,webhookUpdate = false WHERE guild = "${message.guild.id}"`,
					(err) => {
						if (err) throw err;
					}
				);
				...
				db.run(
					`UPDATE logs SET guildBanAdd = false,guildBanRemove = false,guildMemberAdd = false,guildMemberRemove = false,guildMemberUpdate = false,userUpdate = false WHERE guild = "${message.guild.id}"`,
					(err) => {
						if (err) throw err;
					}
				);
			...
			db.all(`SELECT * FROM logs WHERE guild = "${message.guild.id}"`, (err, rows) => {
				if (!rows[0]) return message.channel.send('No logging set up!');
				let columns = rows[0];
				let messageEmbed = {
					embed: {
						...
						fields: [
							{
								...
								value: `channelPinsUpdate ${
									columns['channelPinsUpdate'] ? ':white_check_mark:' : ':x:'
								}
							...
							},
							{
								...
								value: `messageReactionAdd ${
									columns['messageReactionAdd'] ? ':white_check_mark:' : ':x:'
								}
						...
						fields: [
							...
							},
							{
								...
								value: `guildMemberUpdate ${
									columns['guildMemberUpdate'] ? ':white_check_mark:' : ':x:'
								}
