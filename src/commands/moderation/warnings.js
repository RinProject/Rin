...
module.exports = {
	async run(message, args, colors) {
		if (args[1]) {
			...
			if (member)
				db.all(
					`SELECT reason, id, time, active, moderator FROM warnings WHERE guild = ${message.guild.id} AND user = ${member.id}`,
					(err, rows) => {
						if (err)
						...
						rows.forEach((row, i) => {
							let moderator =
								message.guild.members.cache.get(row.moderator).toString() || row.moderator;
							fields.push({
				...
				db.get(
					'SELECT * FROM warnings WHERE guild = (?) AND id = (?)',
