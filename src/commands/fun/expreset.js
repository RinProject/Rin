...
module.exports = {
	async run(message, args) {
        ...
        if (args[1]) {
            if (member) {
                expDB.run(`DELETE FROM exp WHERE guild = "${message.guild.id}" AND user = "${member.id}"`, err=>{if(err) throw err;});
            ...
            expDB.run(`DELETE FROM exp WHERE guild = "${message.guild.id}"`, err=>{if(err) throw err;});
            return message.channel.send('', {
                embed: {
