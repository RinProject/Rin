...
module.exports = {
	run: async function (message, args, colors) {
		if (args[1] && args[1].startsWith('del'))
			return (
				db.run(`DELETE FROM senpais WHERE kouhai = ${message.author.id};`),
				message.channel.send('', {
					embed: { color: 0xff0000, title: 'Senpai cleared' },
				})
			);
		if (message.mentions.users.first())
			db.run(
				`INSERT OR REPLACE INTO senpais(kouhai, senpai) VALUES (${message.author.id}, ${
					message.mentions.users.first().id
				})`
			),
				message.channel.send('Senpai set');
		else
			db.all(`SELECT senpai FROM senpais WHERE kouhai = ${message.author.id}`, (err, rows) => {
				if (err) console.log(err);
					...
					message.channel.send('', {
						embed: {
							color: colors.base,
							title: `Your senpai is ${
								message.client.users.cache.get(rows[0].senpai).username || rows[0].senpai
							}`,
						},
					});
				else
					message.channel.send('', {
						embed: { color: colors.error, title: 'You have no senpai!' },
					});
			});
	},
