    ...
    client.on("message", async (m) => {
        ...
        if(m.guild && !await client.isCommand(m)) {
            ...
            let role = m.guild.roles.resolve((
                await asyncDB.get(db, 
                    "SELECT role FROM expRole WHERE guild = (?);", 
                    [m.guild.id])
            ))
            ...
            let expGain = 10
            let time  = +new Date;
            let xp = await asyncDB.get(db,  
                "SELECT * FROM exp WHERE user =(?) AND guild = (?)",
            ...
            //FIXME <sr229> : This is where using ORM would be really needed, it'll be easier to insert new users...
                ...
                [m.author.id], expGain, time, m.guild.id);
        }
    })
}