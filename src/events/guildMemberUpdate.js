const memberProperties = ['displayHexColor', 'displayName', 'nickname'];

module.exports = (client) => {
	client.on('guildMemberUpdate', (oldMember, newMember) => {
		db.all(
			`SELECT guildMemberUpdate, modLogChannel FROM logs WHERE guild = "${newMember.guild.id}"`,
			(err, rows) => {
				if (rows[0] && rows[0]['guildMemberUpdate']) {
					...
					if (change > 0) {
						oldMember.roles.cache.each((role) => {
							if (!newMember.roles.cache.get(role.id)) editRole = role;
						});
					} else if (change < 0) {
						newMember.roles.cache.each((role) => {
							if (!oldMember.roles.cache.get(role.id)) editRole = role;
						});
					}
					if (editRole)
						embed.fields.push({
