...
db.run(
	'CREATE TABLE IF NOT EXISTS mutes(guild TEXT NOT NULL, member TEXT NOT NULL, ends TEXT NOT NULL, reason TEXT NOT NULL, moderator TEXT NOT NULL);'
);
...
export const mute = {
	mute: async function (
		...
		const mute: Mute | null = await get(
			db,
			'SELECT member FROM mutes WHERE guild = (?) AND member = (?);',
			[guild.id, member.id]
		);
		if (mute && mute.member) throw new Error('User Already Muted');
		const r: Role = await get(db, 'SELECT role FROM muteRole WHERE guild = (?);', [guild.id]);

		let role: Discord.Role;
		if (r) role = guild.roles.resolve(r.role);
		...
		if (!role) {
			...
			await run(db, 'INSERT OR REPLACE INTO muteRole(guild, role) VALUES((?), (?));', [
				guild.id,
				role.id,
			]);
		}
...
async function checkMutes() {
	...
	toUnmute.forEach(async (currentMute) => {
		...
		if (!guild || !member)
			return run(db, 'DELETE FROM mutes WHERE guild = (?) AND member = (?);', [
				currentMute.guild,
				currentMute.member,
			]);
		mute.unmute(guild, member).catch(() => {});
	});
}
