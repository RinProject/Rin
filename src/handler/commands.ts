...
export class Help extends Command {
	...
	constructor(prefix: string, commands: Map<string, Command>, aliases: Map<string, string>, commandData: object[]) {
		super(
			{
				run: async function (message: Discord.Message, args: string[]) {
					...
					if (alias && this.commandAliases.get(alias)) {
						let command = this.commandMap.get(this.commandAliases.get(alias));
						message.channel.send('', {
							embed: {
								title: command.name,
								description: `${command.category?`Category: ${command.category}`:''}Description:\n${command.detailed}\nExample(s): \`${command.examples}\`\n${command.aliases && command.aliases.length ? `Alias(es): \`${command.aliases}\`` : ''}`
							}
						});
						return;
					}
